...
/**
 * Bars visible before the player acts.
 *
 * The slice already contains the outcome — the grader needs it — so the window needs no
 * widening. What it needs is a *narrower start*: the player sees `primeBars` and reveals the
 * rest by playing the replay.
 */
export function primedBarsReplayTrade(level: Level<"replay-trade">): number {
  return level.config.primeBars;
}

 ...
 *
 * The outcome of each trade comes from the grade, not from here — this component never simulates
 * anything, so it cannot know a result before the player has committed to a size for it.
 */
export function TradeSequence({
  ...
  /** The account after the trades the player has already committed to. */
  const runningEquity = overlay
    ? (overlay.steps[at - 1]?.equity ?? equity)
    : equity;
      ...
      <div className="flex flex-wrap gap-x-6 gap-y-1 rounded-lg border border-border bg-surface p-3 font-mono text-xs text-muted">
        ...
        <span aria-live="polite">
          {committed
            ...
            : `trade ${Math.min(at + 1, trades.length)} of ${trades.length}`}
        </span>
        {overlay?.ruined ? (
          ...
          {overlay.steps.map((step, i) => (
            <li
              ...
              className={[
                ...
                overlay.escalations.includes(i)
                  ? "border-down"
                  : "border-border/60",
              ].join(" ")}
            >
